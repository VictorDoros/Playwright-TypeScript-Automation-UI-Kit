...
test.describe('Manage ToDo Items @regression', async () => {
  test('Add, check and delete a todo item @manageItems', async ({ page }) => {
    // #1
    ...
    await test.step('Click on "+" icon to access the "new" todo page', async () => {
      ...
      await expect(
        appPages.createNewToDos.views.createNewToDoPanel,
      ).toBeVisible()

      await expect(
        appPages.createNewToDos.views.createNewToDoHeader,
      ).toHaveText('Create a new Todo')
    })
